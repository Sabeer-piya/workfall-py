...
from wfapi.db_config.config import SECRET_KEY, ALGORITHM
...
def get_current_user(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        user = app_user_queries.findUserByEmail(user_email, db)
        if not user:
            raise HTTPException(status_code=401, detail="User not found")
        print(user.email)
        return user  # or any other data you want to return
